...
const ReminderDialog = ({ isOpen, onClose, invoice, onActionLog, user }) => {
  ...
  const generateReminderText = (templateType, currentInvoiceObject) => {
    if (!currentInvoiceObject) return "";

    const dueDateFormatted = currentInvoiceObject.dueDate ? format(new Date(currentInvoiceObject.dueDate), "d MMMM yyyy", { locale: nl }) : '[Datum]';
    const totalAmount = calculateInvoiceTotal(currentInvoiceObject);
    const invoiceNumber = currentInvoiceObject.id || '[Factuurnummer]';
    const customerName = currentInvoiceObject.receiverName || '[Klantnaam]';
    const invoiceDescription = currentInvoiceObject.invoiceName || '[product/dienst]';
